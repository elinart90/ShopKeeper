...
const WALLET_TYPES = ['business', 'personal', 'savings'];
class WalletsService {
    async ensureWallets(shopId, currency) {
        const { data: existing } = await supabase_1.supabase.from('wallets').select('id').eq('shop_id', shopId);
        if ((existing || []).length >= 3)
            return;
        const names = { business: 'Business Wallet', personal: 'Personal Wallet', savings: 'Savings Wallet' };
        for (const type of WALLET_TYPES) {
            const { data: found } = await supabase_1.supabase
                .from('wallets')
                .select('id')
                .eq('shop_id', shopId)
                .eq('type', type)
                .maybeSingle();
            if (!found) {
                await supabase_1.supabase.from('wallets').insert({
                    shop_id: shopId,
                    type,
                    name: names[type],
                    balance: 0,
    ...
    async getWalletTransactions(shopId, walletId, limit = 50) {
        ...
        if (error) {
            logger_1.logger.error('Error fetching wallet transactions:', error);
    ...
    async adjustBalance(shopId, userId, body) {
        ...
        const { error: txErr } = await supabase_1.supabase.from('wallet_transactions').insert({
            shop_id: shopId,
    ...
    async transfer(shopId, userId, body) {
        ...
        const { error: txOutErr } = await supabase_1.supabase.from('wallet_transactions').insert({
            shop_id: shopId,
        ...
        const { error: txInErr } = await supabase_1.supabase.from('wallet_transactions').insert({
            shop_id: shopId,
