...
class ReportsService {
    ...
    async getComplianceExport(shopId, type, opts) {
        ...
        if (type === 'daily' && opts.date) {
            ...
            if (!m)
                throw new Error('week must be in YYYY-Www format for weekly report');
    ...
    buildForecast(recentDailyMetrics) {
    ...
    computeKpiHealthScore(input) {
        ...
        if (!reasons.length)
            reasons.push('Core KPIs look stable.');
        return { score, status, reasons };
    }
    async getBusinessIntelligence(shopId, _userId, period = 'daily') {
        ...
        const todayVsYesterdayExplanation = Number(currentIntel?.dailyComparison?.profitChangePercent || 0) < 0
        ...
        const trendDetection = {
            revenueTrend: comparison.revenueChangePercent > 5 ? 'up' : comparison.revenueChangePercent < -5 ? 'down' : 'flat',
            profitTrend: comparison.profitChangePercent > 5 ? 'up' : comparison.profitChangePercent < -5 ? 'down' : 'flat',
            narrative: comparison.revenueChangePercent > 5
...
`;
        ...
        return {
            ...
            paymentMethodInsights: {
                mix: paymentMix,
                narrative: this.normalizeCurrencyText(paymentMix.length > 0
                    ? `Top method: ${paymentMix[0].method} (${paymentMix[0].sharePercent.toFixed(1)}%).`
                    : 'No payment method data in selected period.'),
            },
            naturalLanguageDashboardQueryHint: this.normalizeCurrencyText(String(parsed?.engagementHint || 'Ask: "Why is profit down?"')),
            snapshot,
        };
    }
    async queryBusinessIntelligence(shopId, userId, query, period = 'daily') {
...
`;
    ...
    async getInventoryStockIntelligence(shopId, _userId, period = 'weekly') {
        const windows = this.getPeriodWindows(period);
        ...
        const soldQtyByProduct = (salesIntelCurrent?.topProducts || []).reduce((acc, p) => {
        ...
        const payAmount = (obj, key) => Number(obj?.[key]?.amount || 0);
        const currentTotalPay = Object.values(paymentCurrent).reduce((s, p) => s + Number(p?.amount || 0), 0);
        const prevTotalPay = Object.values(paymentPrev).reduce((s, p) => s + Number(p?.amount || 0), 0);
        ...
        const snapshot = {
            ...
            deadStockValue: Number((inventoryFinance?.deadStock || []).reduce((s, d) => s + Number(d.stockValue || 0), 0)),
...
`;
        ...
        return {
            ...
            kpiHealthScore: {
                ...
                reasons: Array.isArray(parsed?.kpiHealthReasons)
                    ? parsed.kpiHealthReasons.map((r) => this.normalizeCurrencyText(String(r)))
                    : [],
            },
            topSellingProducts: snapshot.topSellingProducts,
            lowPerformingProducts: snapshot.lowPerformingProducts,
    ...
    async queryInventoryStockIntelligence(shopId, userId, query, period = 'weekly') {
...
`;
    ...
    getDefaultRangeForIntent(intent) {
        const now = new Date();
        const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    ...
    async callGeminiText(prompt) {
        ...
        const model = env_1.env.geminiModel || 'gemini-2.0-flash';
        const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(env_1.env.geminiApiKey)}`;
    ...
    async callOpenAiThenClaude(prompt) {
        try {
            const text = await this.callOpenAiText(prompt);
            return { provider: 'openai', text };
        }
        catch (openErr) {
            logger_1.logger.warn('OpenAI BI call failed; trying Claude fallback', { message: String(openErr?.message || openErr) });
            const text = await this.callClaudeText(prompt);
            return { provider: 'claude', text };
        }
    }
    async parseIntentWithAi(query) {
...
`;
    ...
    heuristicIntent(query) {
        ...
        if (q.includes('inventory') || q.includes('stock') || q.includes('dead stock'))
            intent = 'inventory_finance';
        else if (q.includes('expense') || q.includes('profit') || q.includes('loss') || q.includes('p&l'))
            intent = 'expenses_profit';
        else if (q.includes('tax') || q.includes('compliance'))
            intent = 'compliance_export';
        else if (q.includes('payment') || q.includes('peak') || q.includes('staff'))
            intent = 'sales_intelligence';
        const language = q.includes('twi') ? 'twi' : 'en';
...
`;
        try {
            ...
            logger_1.logger.warn('Gemini summary failed; using OpenAI fallback', { message: String(geminiErr?.message || geminiErr) });
            const answer = await this.callOpenAiText(prompt);
            return { provider: 'openai', answer };
        }
    }
    async getNaturalLanguageReport(shopId, _userId, query, requestedLanguage = 'auto') {
        ...
        try {
            ...
            intentProvider = (ai.provider || 'openai');
        }
        catch (err) {
            logger_1.logger.warn('AI intent parsing failed; using heuristic', { message: String(err?.message || err) });
            intentResult = this.heuristicIntent(cleanQuery);
            intentProvider = 'heuristic';
        }
        const intent = String(intentResult?.intent || 'dashboard');
        ...
        const inferredLanguage = String(intentResult?.language || 'en').toLowerCase() === 'twi' ? 'twi' : 'en';
        ...
        const chartReferences = intent === 'sales_intelligence'
            ? {
                ...
                points: (rawData?.recentDailyMetrics || []).slice(-14).map((d) => ({
                    x: String(d?.date || ''),
            ...
            : intent === 'expenses_profit'
                ? {
                    ...
                    points: (rawData?.dailyNetProfit || []).slice(-14).map((d) => ({
                        x: String(d?.date || ''),
                        revenue: Number(d?.revenue || 0),
                        expenses: Number(d?.expenses || 0),
                        profit: Number(d?.profit || 0),
